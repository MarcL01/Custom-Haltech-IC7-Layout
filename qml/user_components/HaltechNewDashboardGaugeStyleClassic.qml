...
CircularGaugeStyle {
	...
	*/
    ...
    property real needleLength: toPixels(0.72)
    property real needleTipWidth: toPixels(0.035)
    property real needleBaseWidth: toPixels(0.07)
    ...
    function paintBackground(ctx) {
		ctx.reset();
		   ...
		   //console.log("gaugeFaceColor=" + gaugeFaceColor);
			...
			ctx.arc(xCenter, yCenter, radius * 0.72,
				degToRad(minimumValueAngle + 270),degToRad(valueToAngle(redEndValue) +257 ));
			ctx.stroke();
			...
			if(enableRedStart && redStartValue < gauge.maximumValue) {
				...
				//console.log("redThickness:" + redThickness + ",linewidth:" + ctx.lineWidth);
				ctx.arc(xCenter, yCenter, radius * 0.72,
					degToRad(valueToAngle(redStartValue) +270), degToRad(valueToAngle(redEndValue) +257 ));
				ctx.stroke();
			}
			// Lower Threshold
			if(enableLowerRedStart && gauge.lowerRedStartValue > gauge.minimumValue) {
				...
				ctx.arc(xCenter, yCenter, radius * 0.72,
					degToRad((minimumValueAngle) +270), degToRad(valueToAngle(gauge.lowerRedStartValue) +270 ));
				ctx.stroke();
			}
/**
			// circle
...
**/
		if(circleVisible) {
			...
			ctx.arc(xCenter, yCenter, outerRadius * 0.65,
				degToRad(minimumValueAngle+ 0) , degToRad(maximumValueAngle+ 360));
			ctx.stroke();
		}
		if(circleVisible) {
			ctx.beginPath();
			...
			ctx.arc(xCenter, yCenter, outerRadius * 0.68,
    ...
    background: Canvas {
		...
		Item {
			...
			RadialGradient {
				anchors.fill: parent
				gradient: Gradient {
					GradientStop { position: 0.0; color: "#77777777" }
		...
		Image {
			...
			x: 90
			y: 147
            source: "../../images/classic_screw.png"
        }
		Image {
            id: gaugeScrew2
            width: 8
	...
	/**********************/
	  tickmarkLabel: Text {
			y: 3
			id: tickText
			font.pixelSize: Math.max(1, outerRadius * gauge.labelFontRatio)
